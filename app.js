...
app.use((req, res, next) => {
  ...
  res.locals.testnet = process.env.TESTNET === 'true';
...
app.get('/content/:postId/edit', contentController.editContent);
app.post('/content/create', contentController.postContent);
app.post('/content/:postId/edit', contentController.postEditContent);
...
app.get('/auth/bottlepay', passport.authenticate('bottlepay', { scope: 'user:public wallet:invoice' }));
app.get('/auth/bottlepay/callback', passport.authenticate('bottlepay', { failureRedirect: '/login' }), (req, res) => {
  res.redirect(req.session.returnTo || '/');
});
